...
func TitleCharUpper(str string) string {
	columnItems := strings.Split(str, "_")
...
func TagsKey(columnName string) template.HTML {
	return template.HTML("`gorm:\"primary_key\" json:" + `"` + columnName + "\"`")
	//return template.HTML("`db:" + `"` + columnName + `"` +
	//	" json:" + `"` + columnName + "\"`")
}
