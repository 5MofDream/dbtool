...
var dbName = flag.String("dbName", "test", "the name of db")
var userName = flag.String("userName", "root", "the user name of db")
var pwd = flag.String("pwd", "root", "the password of db")
...
func main() {
	...
	render := template.Must(template.New("model").
		...
		"TitleCharUpper":       modeltool.TitleCharUpper,
	}).
		Parse(string(data)))
...
func genModelFile(db *sqlx.DB, render *template.Template, dbName, dbConnection, tableName string) {
	...
	fileName := *modelFolder + strings.ToLower(tableName) + ".php"
